...
struct tes
{
    char nama[100], usn[50], jk[50], sj[50], jk1[50], sj1[50], status[50], noTelp[50], dtk[50], jnsbr[20], almt[50];
    int lama;
} data;
...
void adminMenu()
{
    ...
    case 1:
        ...
        case 1:
            ...
            dataUser1 = fopen("Data user1.dat", "ab");
            printf("   Masukan Nama yang dicari\t: ");
            gets(cn);
            while (fread(&data, sizeof(data), 1, dataUser) == 1)
                ;
            {
                if (strcmp(data.nama, cn) == 0)
                {
                    printf("%d. Masukkan nama lengkap\t: ", i + 1);
                    fflush(stdin);
                    gets(data.nama);
    ...
    case 3:
        ...
        {
            printf("%d. Nama penjual\t: %s", i + 1, data.nama);
            printf("   Alamat penjual\t: %s ", data.almt);
            printf("   Nomor handphone\t\t: %s ", data.noTelp);
            printf("   Nama kendaraaan yang akan dijual\t: %s", data.jnsbr);
            printf("   Detail produk\t\t: %s", data.dtk);
...
void userMenu()
{
    ...
    case 1:
        ...
        case 1:
            ...
            case 1:
                ...
                fclose(sm1);
                fclose(sm2);
                remove("List kendaraan mobil.dat");
                rename("List kendaraan mobil1.dat", "List kendaraan mobil.dat");
                break;
            case 2:
                while (fread(&data, sizeof(data), 1, sm) == 1)
                {
        ...
        case 2:
            ...
            case 1:
                ...
                fclose(sm1);
                fclose(sm2);
                remove("List kendaraan motor.dat");
                rename("List kendaraan motor1.dat", "List kendaraan motor.dat");
                break;
            case 2:
                while (fread(&data, sizeof(data), 1, sm) == 1)
                {
    ...
    case 2:
        ...
        reqKendaraan = fopen("Request Kendaraan.dat", "ab");
        printf("Banyaknya barang: ");
        scanf("%d", &n);
        getchar();
        for (i = 0; i < n; i++)
        {
            printf("%d. Nama penjual\t: ", i + 1);
            gets(data.nama);
            printf("   Alamat penjual\t: ");
            gets(data.almt);
            printf("   Nomor handphone\t\t: ");
            gets(data.noTelp);
            printf("   Nama kendaraaan yang akan dijual\t: ");
            gets(data.jnsbr);
            printf("   Detail produk\t\t: ");
            gets(data.dtk);
            strcpy(data.status, "pending");
            fwrite(&data, sizeof(data), 1, reqKendaraan);
        }
        fclose(reqKendaraan);
        system("pause");
        main();
